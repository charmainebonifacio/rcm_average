...
' Created By   : Charmaine Bonifacio
'---------------------------------------------------------------------
' Date Edited  : July 22, 2014
' Edited By    : Charmaine Bonifacio
' Comments By  : Charmaine Bonifacio
'---------------------------------------------------------------------
' Organization : Department of Geography, University of Lethbridge
' Title        : SaveFileAs
' Description  : This function saves file as .XLSX or .CSV.
...
' Returns      : -
'---------------------------------------------------------------------
Function SaveFileAs(wbTmp As Workbook, ByVal fileDir As String, _
ByVal Index As String, ByVal fileName As String)
    ...
    ' Save File as Either .XLSX or .CSV
    If Index = 1 Then
        fileExt = ".xlsx"
        FileFormatValue = 51
    End If
    ...
    If CheckFileExists(sPath, saveName) = True Then
        If Dir(sPath & fileName & "_" & saveDate & "_" & i & fileExt) <> "" Then
            Do Until Dir(sPath & fileName & "_" & saveDate & "_" & i & fileExt) = ""
                i = i + 1
            Loop
            saveFile = sPath & fileName & "_" & saveDate & "_" & i
        Else: saveFile = sPath & fileName & "_" & saveDate & "_" & i
        End If
    Else: saveFile = sPath & fileName & "_" & saveDate
    End If
